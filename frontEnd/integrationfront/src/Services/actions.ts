...
export const useApiData = (endpoint: string) => {
    ...
    async function callApi() {
        setLoading(true);
        axios.get(`http://localhost:7260/${endpoint}`,{
          headers:{
  ...
  async function postData(data: any, callback: () => void) {
    setLoading(true);
    axios.post(`http://localhost:5029/${endpoint}`, data,{
      headers:{
        Accept: 'application/json',
       'Content-Type': 'application/json',
    }
  ...
  async function putData(id: number, data: any, callback: () => void) {
    setLoading(true);
    axios.put(`http://localhost:5029/${endpoint}/${id}`, data,{
      headers:{
        Accept: 'application/json',
       'Content-Type': 'application/json',
    }
  ...
  async function deleteData(id: number, callback: () => void) {
    setLoading(true);
    axios.delete(`http://localhost:5029/${endpoint}/${id}`,{
      headers:{
        Accept: 'application/json',
       'Content-Type': 'application/json',
    }
