...
import { useApiData } from '../../Services/actions';
    ...
    type item = { amount: string, name: string, date: string, id: number}
function Model() {
    const [showModal, setShowModal] = useState(false);
    const [brands, setBrands] = useState([]);
    const [selected, setSelected] = useState<item>({ amount: '', name: '', date: '', id: 0 });
    const [mode, setMode] = useState('create');
    const useApiBrand = useApiData('brands');
    const useApi = useApiData('models');
    React.useEffect(() => {
      useApiBrand.callApi();
      useApi.callApi();
    }
    ...
    React.useEffect(() => {
      if(useApiBrand.data){
        const filtered = useApiBrand.data.filter((e: any) => e.name !== '')
  ...
  return (
   ...
   <TEInput
        ...
        <TERipple rippleColor="light" className="w-full mt-2">
          <button
            type="button"
            disabled={selected.amount === '' || selected.name === ''}
            onClick={() => {
              if(selected.amount === '' || selected.name === '') {
                ...
                if(mode === 'create'){
                  ...
                  useApi.postData(data, ()=>{
                    setSelected({ amount: '', name: '', date: '', id: 0});
                    setShowModal(false);
                    useApi.callApi();
                  });
                }
    ...
    <header>
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <h1 className="text-3xl font-bold leading-tight tracking-tight text-gray-900">Balance</h1>
            </div>
          </header>
    <div className='m-2'>
        <button
              ...
              <tbody>
               ...
               {(useApi.data || []).map((row: any) => (
                <tr key={row.id} className="border-b dark:border-neutral-500">
                  ...
                  <td className="whitespace-nowrap px-6 py-4">{helpers.getState(+row.state)}</td>
                            ...
                            <button
                              ...
                              onClick={() => {
                                useApi.deleteData(row.id, () => {
                                  setSelected({ amount: '', name: "", id: 0, date: ""});
                                  useApi.callApi();
                                });
                              }}
